...
let x = 1   // trailing comments are fine`} />
	...
	<section id="identifiers">
		...
		<p>
			Identifiers are letters, digits, and underscores; they cannot start
			with a digit. Visibility is encoded in the first letter:
		</p>
		<ul>
			<li><strong>lowercase first letter</strong> — exported from the module (public).</li>
...
count = count + 1`} />
		...
		<p>
			...
			<a href="#patterns">pattern</a> on the left-hand side.
			Destructuring patterns never include a type prefix or leading
...
let {x, y} = origin      // record pattern; identifiers must match record members`} />
		...
		<p>
			...
			bindings, type definitions, methods, statics, behaviours, tests.
			There are no executable statements at file scope. Every top-level
			right-hand side is therefore evaluated at compile time.
		</p>
	</section>
	...
	<section id="modes">
		<h2>Modes</h2>
		<p>
			Mode prefixes adjust how a binding or parameter is stored, captured,
			or evaluated. The same four prefixes apply in both positions.
		</p>
		<table>
...
let repeat  = (@n: uint, msg) do .. // n must be comptime`} />
		<p>
			...
			default; inside function bodies it is inferred whenever a
			parameter's type is <code>type</code>.
		</p>
	</section>
	...
	<section id="literals">
		...
		<p>
			...
			<code>.</code> default to <code>float</code>. The literal text is
...
let nested = "a\${ "b" + c }d"   // braces and inner strings are balanced`} />
		...
		<p>
			A multiline string opens with <code>|"</code> on its own line and
			closes with <code>|"</code> on its own line. Interior lines begin
...
let nothing = ()    // the unit value, type ()`} />
		...
		<p>
			Tuples are heterogeneous fixed-arity; arrays are homogeneous and
			variable-length. Both literals begin with <code>.</code>, mirroring
			the leading <code>.</code> of record and variant literals — the
...
let typed: [u8] = .{0, 1, 2}   // can also use annotation to pin the element type`} />
		...
		<h3>Records</h3>
		<p>
			A record literal is written as <code>.&#123;...&#125;</code> and the
			record type is inferred from context. Prefix the literal with a
			type name only when no context is available to drive inference.
...
let q = Point.{ x = 1.0, y = 2.0 }    // explicit when there is no context`} />
		...
		<p>
			A variant value is written <code>.tag</code> for a payloadless
...
<CodeBlock code={`let n = nums |> filter(~pred=(x) do x % 2 == 0)
		...
		<p>
			...
			named <code>add</code> makes <code>+</code> available for the type.
		</p>
	</section>
	...
	<section id="patterns">
		...
		<p>
			...
			and <code>for</code> loop binders. Patterns may be <em>refutable</em>
		...
		<p>
			...
			<code>&#123; a, b &#125;</code>. The leading <code>.</code> is
			reserved for value literals.
		</p>
		<table>
		...
		<p>
			Inside variant patterns the payload may itself be a tuple or
			binding pattern — <code>ok((a, b))</code>, <code>some(value)</code>.
		</p>
	</section>
	...
	<section id="control-flow">
		...
		<p>
			...
			<code>do</code>; <code>else</code> is optional and may chain with
			another <code>if</code>. A multi-statement branch uses
			<code>do … end</code>, or <code>do … else …</code> when the
			<code>else</code> closes the branch. The ternary form swaps the
...
while i < 10 do
    ...
    i = i + 1
		...
		<h3>For</h3>
		<p>
			<code>for x in iter do … end</code> — <code>iter</code> must
...
for (k, v) in pairs do ruka.println("\${k}=\${v}")`} />
		...
		<p>
			<code>break</code> and <code>continue</code> apply to the nearest
			enclosing loop. <code>return</code> exits the enclosing function
			with the given value.
		</p>

		<h3>Defer</h3>
		<p>
			<code>defer expr</code> schedules <code>expr</code> to run when the
			enclosing <code>do…end</code> block exits — by reaching the end, by
...
<CodeBlock code={`let report = (h) do
    match h with
		...
		<p>
			...
			<code>else</code> arm covers anything not matched explicitly. A
...
<CodeBlock code={`match n with
	...
	<section id="functions">
		<h2>Functions</h2>
		<p>
			A function literal is <code>(params) do body end</code>. The empty
			parameter list is <code>()</code>. Body forms:
		</p>
		<ul>
...
let main = () do
    ruka.println("\${inc(41)}")
		...
		<p>
			...
			need, or when the annotation is documentation. Parameter modes
			follow the <a href="#modes">mode</a> rules.
		</p>
		<h3>Closures</h3>
		<p>
			Function literals capture their lexical environment. A binding
			declared with <code>&amp;</code> moves into the closure rather than
			being captured by reference.
		</p>
	</section>
	...
	<section id="named-params">
		...
		<p>
			...
			<code>~label=value</code> at the call site and may appear in any
...
greet(~name, ~greeting)`} />
		...
		<p>
			A named parameter typed <code>?(T)</code> is optional. When
			omitted, it receives <code>.none</code>; when given a bare value,
			the value is automatically wrapped in <code>.some(...)</code>.
			This is the one place where annotating a named parameter is
...
greet(~name="Ruka", ~title="Dr.")    // "Dr. Ruka"`} />
		...
		<p>
			A single named argument may follow the closing parenthesis. This
...
let squared = map(nums) ~f=(x) do
	...
	<section id="records-variants">
		...
		<p>
			...
			<code>record &#123; … &#125;</code>. In a multi-line declaration
...
let { x, y } = move(.{ x = 0.0, y = 0.0 })`} />
		...
		<h3>Variants</h3>
		<p>
			A variant (tagged union) names a fixed set of cases, each with an
...
<CodeBlock code={`let Hit = variant {
    ...
    miss
		...
		<p>
			...
			tag is resolved against the variant type that flows in from
			context.
		</p>
	</section>
	...
	<section id="methods">
		...
		<ul>
			<li><code>let name (Type) = …</code> — <strong>static</strong> on <code>Type</code>, called as <code>Type.name(...)</code>.</li>
...
let inc (*self) = () do
    self.count = self.count + 1
		...
		<p>
			...
			the compiler calls it when a value goes out of scope.
		</p>
		<h3>Extending types</h3>
		<p>
			Methods and statics declared in the same module as the type extend
			it. To add methods to a type from outside its declaring module,
			re-export the type and declare methods alongside it; the type
			system will find them by structural lookup.
		</p>
	</section>
	...
	<section id="inference">
		...
		<p>
			Bidirectional inference flows in two directions: a known type
			can drive the shape of an expression (the <em>checking</em>
			direction), and the shape of an expression can determine its
			type (the <em>synthesis</em> direction). Annotations are needed
			only when neither direction reaches a unique answer.
		</p>

		<h3>Numeric defaults</h3>
		<p>
			An integer literal with no contextual type takes <code>int</code>;
			a literal containing <code>.</code> takes <code>float</code>.
			A type-annotated binding or parameter pulls the literal toward a
			more specific numeric type.
		</p>

		<h3>Record literals</h3>
		<p>
			A <code>.&#123;...&#125;</code> record literal first looks for an
			expected type from context — a binding annotation, a parameter
			type, the field type of an enclosing record. If context exists,
			that type wins and the literal is checked against it.
		</p>
		<p>
			With no context, the compiler searches the surrounding scope for
			record types whose field set matches the literal exactly. If
			exactly one type fits, the literal infers to that type. If two or
...
let q = Point.{ x = 1.0, y = 2.0 }   // explicit prefix when ambiguous`} />
		...
		<p>
			...
			is resolved against the expected type from context. With no
			context, the compiler searches scope for variant types that
			declare the tag with a compatible payload. If exactly one matches,
			the constructor infers to that type.
		</p>

		<h3>Record parameters</h3>
		<p>
			A parameter that is used only as a record (field access, record
			destructuring) but has no annotation is inferred structurally.
			The compiler collects every field the body reads and searches
			scope for record types whose declared fields are a superset of
			that set. If exactly one type fits, the parameter takes that
...
let length = (p) do ruka.sqrt(p.x * p.x + p.y * p.y)`} />
		...
		<p>
			...
			<code>match</code>: the compiler collects the tags the arms
			require, and if exactly one variant in scope declares those tags
			(with compatible payload arities), the parameter infers to that
			variant.
		</p>

		<h3>Method receivers</h3>
		<p>
			On a method declared <code>let name (self) = …</code>, the
			receiver type is inferred by the same record-or-variant
			structural rule against the bodies field accesses, mutations,
			and pattern arms.
		</p>

		<h3>Return types</h3>
		<p>
			A function's return type is inferred from its body. Recursive
			functions and mutually recursive groups infer through their
			call graph; an annotation is only needed to break a cycle the
			checker cannot resolve.
		</p>

		<h3>Behaviours never infer</h3>
		<p>
			A behaviour-typed parameter is <strong>never</strong> inferred.
			The behaviour must be written explicitly as the parameter
			annotation. This is what distinguishes a structurally-inferred
			record parameter (concrete type, monomorphic) from a behaviour
...
let area = (s: Shape) do s.area()      // polymorphic over Shape`} />
	...
	<section id="behaviours">
		<h2>Behaviours</h2>
		<p>
			A behaviour declares a set of method signatures. Any type whose
			methods cover those signatures <em>satisfies</em> the behaviour —
			no <code>implements</code> declaration is required. Like records
...
<CodeBlock code={`let Shape = behaviour {
    ...
    perimeter(self): () -> f64
		...
		<p>
			A behaviour may appear as a parameter type. Each call site
...
<CodeBlock code={`let describe = (s: Shape) do
    ruka.println("area: \${s.area()}")
		...
		<p>
			...
			behaviours describe behaviour, not storage.
		</p>
...
<CodeBlock code={`let iterable = behaviour {
    next(*self): () -> ?(T)
		...
		<p>
			...
			<code>lt</code>, <code>index</code> are recognised as operator
			implementations. Defining one of these enables the corresponding
	...
	<section id="modules">
		...
		<p>
			A Ruka file <em>is</em> a module — its top-level declarations are
			the module's fields. Lowercase-named declarations are exported;
			uppercase-named declarations are private.
		</p>
		<p>
			<code>ruka.import("path")</code> evaluates at compile time and
...
describe(animals.Dog)`} />
	...
	<section id="tests">
		<h2>Tests</h2>
		<p>
			A <code>test</code> binding declares a zero-argument function that
			runs as part of the test suite. Tests are compiled in
...
test addition = () do
    ...
    ruka.expect_eq(add(0, 0), 0)
		...
		<p>
			...
			separate test-visibility mechanism.
		</p>
	</section>
	...
	<section id="comptime">
		...
		<p>
			...
			<code>@</code>-prefixed parameters run at compile time;
			their results are compile-time constants. Types, functions, and
			modules are first-class values in compile-time contexts.
		</p>
...
let min = (t,        a: t, b: t) do if a < b do a else b`} />
		...
		<h3>Generics</h3>
		<p>
			Each unique compile-time argument set produces a distinct
...
let FloatBuf = FixedArray(f64, 16)`} />
		...
		<p>
			At file scope, <code>let</code> already evaluates its right-hand
			side at compile time, so no prefix is needed. Inside a function
...
let run = () do
    ...
    let rows   = fetch_rows()              // runtime
		...
		<h3>Reflection</h3>
		<p>
			<code>ruka.type_of(e)</code> returns the type of <code>e</code> as
			a compile-time value. <code>ruka.fields_of</code>,
			<code>ruka.methods_of</code>, and <code>ruka.statics_of</code>
			return the structural pieces of a type.
			<code>ruka.record_of(fields)</code> constructs a new record type
			from a list of field descriptors. All four take
...
// PartialUser ≡ record { id: ?(i32), name: ?(string) }`} />
	...
	<section id="ruka-module">
		...
		<p>
			<code>ruka</code> is the prelude — always in scope, never
			imported. The members below are referenced throughout this
			document.
		</p>
		...
		<table>
			...
			<tbody>
				<tr><td><code>print(...)</code></td><td>Write each argument's <code>format</code> to stdout, no separator, no newline.</td></tr>
				<tr><td><code>println(...)</code></td><td>Like <code>print</code>, joins with spaces, appends a newline.</td></tr>
				<tr><td><code>read()</code> / <code>readln()</code></td><td>Read a line of input from stdin and return it as a <code>string</code>.</td></tr>
			</tbody>
		</table>
