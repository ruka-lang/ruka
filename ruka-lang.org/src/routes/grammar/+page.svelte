...
<DocsShell title="Grammar" {sections}>
	<h1>Grammar</h1>
	<p>
		This page is the formal grammar of Ruka, written in a variant of
...
<pre class="ebnf"><code>rule-name  ::=  production</code></pre>
		...
		<p>
			Lexical rules describe raw text; syntactic rules describe token
			sequences. Both use lowercase-with-hyphens names. Whitespace and
			comments between tokens are always permitted and are omitted from
			syntactic rules for clarity.
		</p>
	</section>
	...
	<section id="lexical">
		...
		<p>
			The lexical grammar describes how raw source text is divided into
			tokens. The scanner reads left-to-right and always produces the
			longest possible token at each position (maximal munch).
		</p>
		...
		<p>
			Whitespace and comments are discarded by the scanner and do not
			appear in the syntactic grammar. Block comments are not supported.
		</p>
<pre class="ebnf"><code>whitespace    ::=  ( " " | "\t" | "\r" | "\n" )+

comment       ::=  "//" &lt;any char except "\n"&gt;* "\n"?</code></pre>

		<h3 id="identifiers">Identifiers</h3>
		<p>
			An identifier begins with a letter or underscore and is followed by
			zero or more letters, digits, or underscores. Any identifier that
			matches a keyword or mode keyword is reserved and cannot be used as
			a user-defined name. The first letter of an identifier determines
			visibility at file scope: lowercase exports, uppercase is private.
		</p>
<pre class="ebnf"><code>letter        ::=  [a-zA-Z_]
digit         ::=  [0-9]

identifier    ::=  letter ( letter | digit )*
                   -- must not be a keyword or mode keyword</code></pre>
		...
		<p>
			The following symbols are reserved as <em>mode prefixes</em>. A
			mode prefix is placed directly before a parameter or binding
			identifier with no whitespace between the prefix and the
			identifier.
		</p>
<pre class="ebnf"><code>mode-prefix   ::=  "*"    -- mutable; binding may be reassigned, parameter mutates in place
               ...
               |   "@"    -- compile-time; value must be known at compile time</code></pre>
		<p>
			<code>self</code> is reserved for the method receiver; it may only
			appear in the receiver clause of a binding declaration or as a
			parameter inside a method body. <code>with</code> is used only as
			part of <code>match</code> syntax. <code>ruka</code> is a reserved
			identifier referring to the built-in module; it is implicitly in
			scope in every source file and cannot be shadowed or rebound.
		</p>

		<h3 id="literals-lex">Literals</h3>

		<h4>Integer literals</h4>
<pre class="ebnf"><code>decimal-digit ::=  [0-9]
hex-digit     ::=  [0-9a-fA-F]
binary-digit  ::=  "0" | "1"
...
integer-lit   ::=  decimal-lit | hex-lit | binary-lit
                   ...
                   -- default type: int; overridden by type context</code></pre>

		<h4>Float literals</h4>
<pre class="ebnf"><code>float-lit     ::=  decimal-digit+ "." decimal-digit+
                   -- leading minus is a unary operator
                   -- default type: float; overridden by type context</code></pre>
...
<pre class="ebnf"><code>char-escape   ::=  "\\" ( "n" | "t" | "r" | "\\" | "'" | '"' | "0" )
char-lit      ::=  "'" ( &lt;any char except "'", "\\", "\n"&gt; | char-escape ) "'"</code></pre>

		<h4>String literals</h4>
		<p>
			String literals are delimited by double quotes. Expressions may be
			embedded with <code>&#36;&#123;…&#125;</code>; the interpolated expression must
			satisfy the <code>ruka.printable</code> behaviour.
		</p>
<pre class="ebnf"><code>str-escape    ::=  "\\" ( "n" | "t" | "r" | "\\" | "'" | '"' | "0" )
str-interp    ::=  "&#36;&#123;" expr "&#125;"
str-content   ::=  ( &lt;any char except '"', "\\", or "$"&gt; | str-escape | str-interp )*
string-lit    ::=  '"' str-content '"'</code></pre>

		<h4>Multiline string literals</h4>
		<p>
			Multiline strings begin with <code>|"</code> followed by a newline.
			Each content line must begin with <code>|</code>, which is stripped
			from the resulting string value (a single space immediately after
			the bar is also stripped). The literal ends with <code>|"</code> on
			its own line.
		</p>
<pre class="ebnf"><code>ml-content    ::=  &lt;any char except "\n"&gt;
ml-line       ::=  "|" ml-content* "\n"
multiline-lit ::=  '|"' "\n" ml-line* '|"'</code></pre>
	</section>
	...
	<section id="syntactic">
		...
		<p>
			...
			and comments between any two tokens are implicitly permitted and
			ignored. A few rules below refer explicitly to <code>newline</code>
			as a structural marker — these are the cases where the scanner's
			line breaks become syntactically significant (single-expression
			block bodies).
		</p>

		<h3 id="program">Program</h3>
		<p>
			A Ruka source file is a flat sequence of declarations. Every file
			is implicitly a compile-time record: top-level <code>let</code>
			declarations whose names begin with a lowercase letter become its
			public fields; those whose names begin with an uppercase letter are
			private fields, inaccessible to importers.
			<code>ruka.import</code> returns this record value; there is no
...
<pre class="ebnf"><code>program       ::=  declaration*</code></pre>
		...
		<p>
			There are two declaration forms: ordinary <code>let</code> bindings
			and <code>test</code> bindings. There is no separate <code>fn</code>,
			<code>type</code>, or <code>mod</code> keyword. Mutability and
			evaluation time are expressed through modes; privacy is determined
			by the case of the binding name.
		</p>
<pre class="ebnf"><code>declaration   ::=  binding | test-binding
...
test-binding  ::=  "test" identifier "=" expr
                   -- the value must be a function expression; compiled in debug/test builds only</code></pre>
		<p>
			<strong>Uniform declarations.</strong> Functions, types,
			behaviours, and modules are all values stored in ordinary
			<code>let</code> bindings. A <em>receiver</em> in the binding
			left-hand side associates the value with a named type as either a
			static function (type-name receiver) or a method (<code>self</code>
			receiver). The receiver appears between the binding name and the
			<code>=</code> sign.
		</p>
		<p>
			<strong>Type extension.</strong> The type named by a receiver is
			not required to be declared in the current scope — any type is a
			valid receiver, including primitives (<code>i32</code>,
			<code>bool</code>, etc.) and built-in generics. An extension
			declared outside the type's original scope shadows the type within
			the extending scope rather than mutating it. See
			<a href="/reference#methods">Methods &amp; statics</a>.
		</p>
		<p>
			<strong>Privacy.</strong> Visibility is determined by the first
			letter of the binding name — the inverse of Go's convention. A
			binding whose name begins with a lowercase letter is public
			(exported from the file record); a binding whose name begins with
			an uppercase letter is private. The same rule applies to methods,
			statics, and record or variant fields. Privacy does not apply
			inside function bodies, where name case is purely conventional.
		</p>
		<p>
			<strong>Mutability.</strong> By default a <code>let</code> binding
			is immutable. The <code>*</code> mode prefix makes the binding a
			mutable variable: <code>let *count = 0</code>. Reassignment is only
			permitted on bindings declared with <code>*</code>.
		</p>
		<p>
			<strong>Evaluation time.</strong> Bindings at file scope (including
			methods, statics, and type declarations) are implicitly
			compile-time. The <code>@</code> mode may be written explicitly but
			is redundant there. Inside an inner scope (function body, block) a
			plain <code>let</code> is runtime; <code>@</code> must be written
			explicitly to force compile-time evaluation of the right-hand side.
		</p>
		<p>
			<strong>Test bindings.</strong> A <code>test</code> binding
			declares a function that is only compiled in debug and test builds
			and elided entirely in optimised builds. The value must be a
			function expression. Test bindings have no visibility qualifier and
			no receiver clause. The name is an ordinary identifier used by the
			test runner to identify the test.
		</p>

		<h3 id="types">Types</h3>
		<p>
			Type expressions appear after <code>:</code> in parameter and
			binding annotations, after <code>-&gt;</code> in return-type
			annotations, and as values passed to <code>type</code>-typed
			parameters.
		</p>
<pre class="ebnf"><code>type          ::=  "()"                              -- unit type
               |   primitive-type
               |   array-type
               |   tuple-type
               |   range-type
...
field-sep     ::=  newline | ","                     -- newline separates members; comma only required on a single line</code></pre>
		<p>
			<strong>Result-location semantics.</strong> The collection literal
			<code>.&#123;…&#125;</code> produces an array, and
			<code>.(…)</code> produces a tuple. The element type of an array
			literal and the field types of a record literal flow in from
			context; an annotation pins them when context is unavailable.
		</p>

		<h3 id="expressions">Expressions</h3>
		<p>
			Ruka is expression-based: blocks, conditionals, match expressions,
			and function bodies all evaluate to a value. The following layered
			grammar encodes operator precedence structurally, from
			lowest-binding (outermost rule) to highest-binding (innermost
			rule). The <a href="/reference#operators">reference's operator
			table</a> presents the same precedence in tabular form.
		</p>
<pre class="ebnf"><code>expr          ::=  ternary-expr
...
unary-expr    ::=  "not" unary-expr
               |   "-" unary-expr
               |   postfix-expr</code></pre>

		<h3 id="operators">Operator precedence</h3>
		<p>
			The table below summarises all operators from lowest to highest
			precedence.
		</p>
		<table>
			<thead>
			...
			<tbody>
				...
				<tr><td>16 (highest)</td><td><code>. &nbsp; [] &nbsp; () &nbsp; .?() &nbsp; .!()</code></td><td>Left</td><td>Field access, index, call, unwrap</td></tr>
			</tbody>
		</table>

		<h3 id="postfix">Postfix expressions</h3>
		<p>
			Postfix operations bind tighter than any prefix or infix operator
			and are left-associative, allowing arbitrary chaining.
		</p>
<pre class="ebnf"><code>postfix-expr  ::=  primary postfix-op*
...
trailing-arg  ::=  "~" identifier "=" function-expr
                   ...
                   -- e.g.  map(nums) ~f=(x) do x * 2</code></pre>

		<h3 id="primary">Primary expressions</h3>
<pre class="ebnf"><code>primary       ::=  literal-expr
...
variant-ctor  ::=  "." identifier                         -- unqualified, no payload: .tagName
               ...
               |   identifier "." identifier "(" expr ")" -- qualified, with payload: Type.tagName(expr)</code></pre>

		<h3 id="functions">Function expressions</h3>
		<p>
			Functions are anonymous values. A function expression is a
			parameter list, an optional return-type annotation, and a body
			introduced by <code>do</code>. The body uses the same
			<code>block-expr</code> rule as every other block.
		</p>
<pre class="ebnf"><code>function-expr ::=  "(" param-list? ")" return-annot? block-expr
...
type-annot    ::=  ":" type</code></pre>
		<p>
			<strong>Receiver and function expression.</strong> When a binding
			declaration carries a receiver clause, the
			<code>param-list</code> describes only the <em>explicit</em>
			parameters — the receiver itself is declared by the binding, not
			by the function expression. See
		...
		<p>
			...
			<em>named parameter</em>. Named parameters are always passed by
			label at the call site and may appear in any order. A named
			parameter may also be passed as a trailing argument outside the
			closing parenthesis, which is useful for higher-order functions
			that accept a closure.
		</p>
<pre class="ebnf"><code>-- Declaration (in param-list)
...
--         end</code></pre>
		...
		<p>
			All control flow constructs are expressions and produce a value.
			When used purely for side effects the result is the unit type
			<code>()</code>. Each construct's body is a <code>block-expr</code>
			(see Block expression above) — the single-line and multi-line
			forms apply uniformly.
		</p>

		<h4>If expression</h4>
		<p>
			A multi-statement <code>if</code> chain is closed by a single
			trailing <code>end</code>; an all-single-expression chain has no
			<code>end</code>, since each branch closes at its newline.
		</p>
<pre class="ebnf"><code>if-expr       ::=  "if" expr block-expr
                   ( "else" "if" expr block-expr )*
                   ( "else" block-expr )?
                   -- when any branch uses the multi-statement block-expr form, the chain ends with "end"
                   -- when every branch is single-expression, the chain has no "end"</code></pre>
		<p>
			<strong>Conditional expression (ternary).</strong> A right-hand-side
			conditional uses <code>value if cond else value</code> — the same
			keywords, rearranged. The form sits at the top of the expression
			hierarchy (just below <code>expr</code>), is right-associative on
			the <code>else</code> branch, and parses the <code>cond</code> at
			<code>or-expr</code> precedence — a nested ternary in the
			condition position requires parentheses.
		</p>
<pre class="ebnf"><code>ternary-expr  ::=  assign-expr ( "if" or-expr "else" ternary-expr )?
...
<pre class="ebnf"><code>for-expr      ::=  "for" for-pattern "in" expr block-expr
...
for-pattern   ::=  identifier                       -- simple binding
               ...
               |   record-pattern                   -- "&#123; a, b, ... &#125;" record destructure</code></pre>

		<h4>Return</h4>
<pre class="ebnf"><code>return-expr   ::=  "return" expr
                   -- "return" always carries a payload expression
                   -- a unit-returning function exits early with `return ()`</code></pre>

		<h4>Break and continue</h4>
		<p>
			<code>break</code> and <code>continue</code> are only valid inside
			a <code>while</code> or <code>for</code> loop body. Both produce
			the unit value.
		</p>
<pre class="ebnf"><code>break-expr    ::=  "break"
                   -- immediately exits the innermost enclosing loop

continue-expr ::=  "continue"
                   -- skips the remainder of the current iteration and begins the next</code></pre>

		<h3 id="defer">Defer</h3>
		<p>
			A <code>defer</code> statement schedules an expression to execute
			at the end of the enclosing <code>do…end</code> block, regardless
			of how control exits that block. Multiple defers in the same block
			execute in LIFO order (last deferred, first to run).
		</p>
<pre class="ebnf"><code>defer-stmt    ::=  "defer" expr
                   -- expr is evaluated when the enclosing block exits
                   -- expr is typically a function call or a do...end block
                   -- defers in the same block run LIFO: last defer statement runs first</code></pre>
		...
		<p>
			...
			(may not match) or <em>irrefutable</em> (always match) — only
			irrefutable patterns are allowed in <code>let</code> and
			<code>for</code>.
		</p>
		<p>
			<strong>No leading dot in patterns.</strong> The leading
			<code>.</code> of <code>.tag</code>, <code>.&#123;…&#125;</code>, and
			<code>.(…)</code> belongs to <em>value literals</em>; in patterns
			the same shapes are written bare.
		</p>
<pre class="ebnf"><code>match-expr    ::=  "match" expr "with" arm+ else-arm? "end"
...
guard-pattern ::=  pattern "if" expr
                   -- e.g.  n if n &lt; 0   in   match n with  n if n &lt; 0 do "negative" ... end</code></pre>
		<p>
			<strong>Option and result patterns.</strong> <code>?(T)</code> and
			<code>!(T, E)</code> are built-in variant types and follow the
			same variant-pattern syntax: <code>some(name)</code>,
